...
bool Value::equals(const Value& o) const {
    ...
    switch (kind) {
        ...
        case VarKind::Float:
        case VarKind::Double: return d == o.d; // bitwise/value equality is fine for change detection
...
// ---------------------------------------------------------------------------
// clamp_to_meta — only meaningful for numeric kinds with min < max
// ---------------------------------------------------------------------------

Value clamp_to_meta(const Value& in, const VarMeta& meta) {
    if (meta.min >= meta.max) return in; // no-range hint
    Value out = in;
    auto clamp_d = [&](double x) {
        return std::min(meta.max, std::max(meta.min, x));
    };
    switch (in.kind) {
        case VarKind::Int32:
        case VarKind::Int64:
            out.i = static_cast<int64_t>(clamp_d(static_cast<double>(in.i)));
            break;
        case VarKind::Float:
        case VarKind::Double:
            out.d = clamp_d(in.d);
