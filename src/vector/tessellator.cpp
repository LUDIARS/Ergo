...
static std::vector<Ring> contours(const VectorPath& p) {
...
void tessellate_path(const VectorPath& path, std::vector<Vec2>& out_v, std::vector<uint32_t>& out_i) {
  ...
  if (cs.empty()) return;
  std::vector<Ring> poly{cs.front()};
  auto idx = mapbox::earcut<uint32_t>(poly);
  uint32_t base = static_cast<uint32_t>(out_v.size());
  for (auto& p : cs.front()) out_v.push_back({static_cast<float>(p[0]), static_cast<float>(p[1])});
