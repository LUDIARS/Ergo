...
| `ergo_http`         | libcurl バックエンドの同期 HTTP クライアント。`IHttpClient` / `get` / `post`。公開 API は `std::` のみで curl を隠蔽。既定 OFF (`ERGO_WITH_CURL` を強制) | システム | `include/ergo/http/`         | `src/http/`         | `spec/module/http.md`         | (依存: `curl`) |
...
| `ergo_stickfig`    | 棒人間 (頭=球 / 四肢=カプセル) のプロシージャル幾何生成。CPU メッシュのみ・Vulkan 非依存。出力頂点は `StageVertex` 互換 | ゲームオブジェクト | `include/ergo/stickfig/`    | `src/stickfig/`    | `spec/module/stickfig.md`    | `tools/ergo/` (`stickfig` plugin) |
