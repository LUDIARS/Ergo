...
| `ergo_sound`        | WAV/OGG デコード + ストリーミング + ミキサ + 波形処理 + Quantizer                      | システム | `include/ergo/sound/`        | `src/sound/`        | `spec/module/sound.md`        | — |
...
- **`ergo_sound` の OGG デコーダ**: 現状はヘッダ解析のみで実 PCM デコードは
  未実装。`spec/module/sound.md` のテスト観点にも「OGG は完全実装されて
  いない」と明記すべき (TODO)。
