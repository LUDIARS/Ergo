...
> 関連: [ui_layout.md](./ui_layout.md) (このモジュールの主要 consumer = vector ノード), PrivateGame `spec/hud_svg.md` (最初の実利用), [render.md](./render.md), Pictor UIRenderer。

## 0. 背景と目的

PrivateGame の HUD を「SVG + アニメーション」で作りたいが、LUDIARS のレンダ基盤には **SVG を直接描く経路が無い**。Pictor のベクター標準は Rive (.riv) だが、Rive は実行時の数値/テキスト data-binding が弱く、また素材を Rive エディタで .riv 化する人手工程が必要。
