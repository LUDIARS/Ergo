...
```
Pictor / AdventureCube / PrivateGame (C++ アプリ)
...
- アプリ側 (Pictor demo / AdventureCube / PrivateGame) が
...
- 採用理由: Dear ImGui / RmlUi を選ばず自作 (decision-metrics 評価表は §10 参照)
  - Ergo 側に primitives を集約することで、 Pictor / AdventureCube / PrivateGame 等
...
- アプリの main loop に組み込む形:
  ...
  ```cpp
  // Pictor demo / AdventureCube / PrivateGame の main 等から
...
- apps.json 駆動の build/run/test を本物の AdventureCube / PrivateGame で実行 (CI 上では skip flag)
...
2. 複数アプリ (Pictor demo / AdventureCube / PrivateGame) で同じ in-proc UI を再利用可能
