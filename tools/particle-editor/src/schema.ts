...
///   - public/index.html  (DEFAULTS / SCHEMA constants)
///   - ergo_particle      (ParticleEffectConfig struct in C++)

export const SCHEMA_VERSION = 1;

export interface Color4 { 0: number; 1: number; 2: number; 3: number; length: 4 }
...
// ---- Wire protocol ---------------------------------------------------------
//
// All frames are JSON over WebSocket text.
//
// server -> client (on connect, and after any update):
...
//   { "op": "ping" }                                                // optional liveness
//
// server -> client (response/event):
//   { "op": "ack" }                                                 // optional
//
// Engine clients (e.g. AdventureCube) usually only receive `state`;
// browser clients both send `set` (on UI changes) and receive `state`
